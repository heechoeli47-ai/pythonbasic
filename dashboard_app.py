...
from PyQt5.QtWidgets import (
    ...
    QTableWidget, QTableWidgetItem, QHeaderView, QTextEdit, QGroupBox, QGridLayout, QMessageBox, QSplitter
)
...
class TradeItem:
    ...
    reentry_block_until_bucket: int = -1
...
class AutoTrader(QWidget):
    ...
    def __init__(self):
        super().__init__()
        self.logs = []  # 메모리 로그 버퍼
        self.txt_logs = None  # UI 로그창 생성 전 임시 버퍼
        ...
        self.exit_lock_sec = 3.0
        self.reentry_cooldown_bars = 8
        self.market_regime_threshold = 0.52
        self.volume_ratio_min = 1.30
        ...
        self.market_up = 0
        self.market_down = 0
        self.market_up_ratio = 0.5
        self.daily_perf_done = False
        self.charts_ready = False
        # 계좌 보유종목 자동 동기화 + 보유종목 전용 Trade List 모드
        self.sync_balance_holdings_to_trade_list = True
        self.trade_list_holdings_only_mode = True
        ...
        self.add_buy_drop_pct = 0.02       # 평단가 대비 2% 이상 하락 시 2차 매수

        # candles settings
        self.candle_sec = 3
        self.max_candles_3s = 1200  # 3초봉 최대 보관 개수
        self.max_bars_3m = 600       # 3분봉 최대 보관(약 30시간)
        ...
        self.basic_timer.start(80)  # 기본정보 TR 큐 타이머 시작(80ms)
        self.apply_tr_q = []
    ...
    def _build_ui(self):
        ...
        DEBUG_UI_CLICKS = True  # 클릭 로그를 보려면 True
        ...
        self.btn_pick_checked = QPushButton("1) Pick -> Tr.L")
        ...
        self.btn_set_real = QPushButton("2) SetReal + TR(P)")
        self.btn_auto_trade = QPushButton("3) Auto OFF")
        ...
        self.ed_market_regime = QLineEdit("0.52")
        self.ed_volume_ratio = QLineEdit("1.30")
        self.ed_max_chase_pct = QLineEdit("2.0")
        self.ed_reentry_cooldown = QSpinBox()
        self.ed_reentry_cooldown.setRange(0, 30)
        self.ed_reentry_cooldown.setValue(8)
        self.ed_exit_lock_sec = QLineEdit("3.0")
        ...
        r += 1
        cg.addWidget(QLabel("Regime Threshold"), r, 0)
        cg.addWidget(self.ed_market_regime, r, 1)
        r += 1
        cg.addWidget(QLabel("Volume Ratio Min"), r, 0)
        cg.addWidget(self.ed_volume_ratio, r, 1)
        r += 1
        cg.addWidget(QLabel("Max Chase (%)"), r, 0)
        cg.addWidget(self.ed_max_chase_pct, r, 1)
        r += 1
        cg.addWidget(QLabel("Reentry Cooldown Bars"), r, 0)
        cg.addWidget(self.ed_reentry_cooldown, r, 1)
        r += 1
        cg.addWidget(QLabel("Exit Lock (sec)"), r, 0)
        cg.addWidget(self.ed_exit_lock_sec, r, 1)
        r += 1
        ...
        config_group.setMaximumWidth(900)
        ...
        summary_group.setMinimumWidth(210)
        summary_group.setMaximumWidth(240)
        ...
        # 상단 헤더는 전체폭, 본문은 좌/우 분할
        config_container = QWidget()
        config_container.setLayout(config_row)
        config_container.setMinimumHeight(320)
        cand_group.setMinimumHeight(250)
        trade_group.setMinimumHeight(180)

        left_split = QSplitter(Qt.Vertical)
        left_split.addWidget(config_container)
        left_split.addWidget(cand_group)
        left_split.addWidget(trade_group)
        left_split.setStretchFactor(0, 3)
        left_split.setStretchFactor(1, 4)
        left_split.setStretchFactor(2, 3)
        left_split.setSizes([350, 320, 250])

        right_split = QSplitter(Qt.Vertical)
        right_split.addWidget(self.chart_widget)
        right_split.addWidget(self.txt_logs)
        right_split.setMinimumWidth(820)
        right_split.setMaximumWidth(940)
        right_split.setStretchFactor(0, 6)
        right_split.setStretchFactor(1, 4)
        right_split.setSizes([560, 340])
    ...
    def _is_holdings_only_mode(self) -> bool:
        return bool(getattr(self, "sync_balance_holdings_to_trade_list", False)) and bool(
            getattr(self, "trade_list_holdings_only_mode", True)
        )

    def _prune_non_holding_trade_items(self, reason: str = "holdings-only-prune"):
        if not hasattr(self, "trade_map") or not self.trade_map:
            return
        targets = [
            code for code, t in self.trade_map.items()
            if int(getattr(t, "pos_qty", 0) or 0) <= 0 and not bool(getattr(t, "manual_picked", False))
        ]
        if not targets:
            return
        removed, skipped = self._remove_trade_codes(targets, reason=reason, block_holding=True)
        if removed > 0 or skipped > 0:
            self.log(
                f"[TR.L HOLDINGS ONLY] removed_non_holding={removed} skipped_holding={skipped} total={len(self.trade_map)}"
            )
    ...
    def _recalc_per_stock_budget(self):
        """
        PerStock 예산은 실제 Trade List(=trade_map) 종목 수로 나눔.
        PickN은 후보 선택 개수 제한용이며 예산 분배 계산에는 직접 반영하지 않음.
        """
    ...
    def _apply_button_styles(self):
        ...
        self._style_button(self.btn_auto_trade, "#f1c40f")  # 자동매매 토글 버튼 스타일
    ...
    def _calc_account_totals_from_trade_map(self):
        ...
        for code, t in self.trade_map.items():
            ...
            last_raw = getattr(t, "last", 0) or getattr(t, "last_price", 0) or avg
    ...
    def _on_pick_checked(self):
        if self._is_holdings_only_mode():
            self.log("[PICK] holdings mode ON: 보유종목 + 오늘 Pick 종목을 함께 표시/유지합니다.")
    ...
    def _on_setrealreg_and_tr_picked(self):
        ...
        2) 기본정보(opt10001) + 3분봉(opt10080) 부트스트랩 TR 순차 요청
        """
    ...
    def _on_toggle_auto_trade(self):
        ...
        if hasattr(self, "btn_auto_trade"):
            self.btn_auto_trade.setText("3) Auto ON" if nxt else "3) Auto OFF")
    ...
    def _update_account_pnl_ui(self, total_profit=None, total_buy=None, total_eval=None):
        # 숫자/문자 혼합 입력에서도 UI가 항상 안정적으로 갱신되도록 정규화
        total_buy = int(to_float(total_buy, 0.0)) if total_buy is not None else 0
        total_eval = int(to_float(total_eval, 0.0)) if total_eval is not None else 0
        if total_profit is None:
            total_profit = total_eval - total_buy
        else:
            total_profit = int(to_float(total_profit, 0.0))
    ...
    def _apply_config(self):
        ...
        self.market_regime_threshold = min(0.95, max(0.0, to_float(self.ed_market_regime.text(), 0.52)))
        self.volume_ratio_min = max(1.0, to_float(self.ed_volume_ratio.text(), 1.30))
        self.max_chase_pct = max(0.0, to_float(self.ed_max_chase_pct.text(), 2.0) / 100.0)
        self.reentry_cooldown_bars = max(0, int(self.ed_reentry_cooldown.value()))
        self.exit_lock_sec = max(0.5, to_float(self.ed_exit_lock_sec.text(), 3.0))
        self.log(f"[CONFIG] RSI Buy Level set to {self.rsi_buy_level}")
        self.log(
            f"[CONFIG] regime_th={self.market_regime_threshold:.2f} "
            f"vol_ratio={self.volume_ratio_min:.2f} "
            f"max_chase={self.max_chase_pct*100:.2f}% "
            f"reentry_cd={self.reentry_cooldown_bars} "
            f"exit_lock={self.exit_lock_sec:.1f}s"
        )
    ...
    def _per_stock_budget(self) -> int:
        """
        종목당 예산은 실제 Trade List 종목 수로 나눔.
        spn_topn(후보 수)은 여기서 직접 사용하지 않음.
        """
    ...
    def _on_click_login(self):
        try:
            ...
            if state == 1:
                ...
                self._reset_trade_list_for_new_session()
                self._load_accounts()
                QTimer.singleShot(300, self._load_conditions)
                QTimer.singleShot(800, self._request_balance_opw00018)
                return
    ...
    def _reset_trade_list_for_new_session(self):
        prev = len(self.trade_map) if hasattr(self, "trade_map") and self.trade_map else 0
        if hasattr(self, "trade_map"):
            self.trade_map.clear()
        self.display_code = None
        self._render_trade_list()
        self._update_account_summary_program()
        self._refresh_charts()
        self.log(f"[SESSION] trade list reset on login (cleared={prev})")
    ...
    def _ensure_tradeitem_from_balance(self, code: str, name: str, qty: int, avg: float, cur: int):
        """
        opw00018 보유종목을 trade_map과 UI에 동기화.
        - trade_map에 없으면 생성
    ...
    def _on_receive_condition_ver(self, ret, msg):
        ...
        self.log(f"[COND] raw='{raw_s[:120]}'")  # 너무 길면 앞부분만
        if not raw_s:
    ...
    def _render_trade_list(self):
        ...
        for code, t in self.trade_map.items():
            if (
                self._is_holdings_only_mode()
                and int(getattr(t, "pos_qty", 0) or 0) <= 0
                and not bool(getattr(t, "manual_picked", False))
            ):
                continue
            row = self.tbl_trade.rowCount()
            self.tbl_trade.insertRow(row)
    ...
    def _check_market_strength(self):
        ...
        try:
            ...
            tot = up + down
            self.market_up_ratio = (float(up) / float(tot)) if tot > 0 else 0.5
    ...
    def _update_account_summary_program(self):
        ...
        # 당일 누적 매입금(프로그램 집계) + 보유기준 매입금(재기동/잔고동기화 보완)
        total_buy_program = int(getattr(self, "today_total_spent", 0) or 0)
        total_buy_holdings = 0
        ...
        for code, t in self.trade_map.items():
            ...
            qty = int(getattr(t, "pos_qty", 0))
            avg = float(getattr(t, "avg_price", 0))
            mark = float(getattr(t, "last", 0) or getattr(t, "last_price", 0) or avg)

            if qty > 0 and avg > 0:
                total_buy_holdings += qty * avg

            if qty > 0 and mark > 0:
                eval_amt = qty * mark
                total_eval += eval_amt
                unreal_pnl += (mark - avg) * qty

        total_buy = total_buy_program if total_buy_program > 0 else int(total_buy_holdings)
    ...
    def _handle_balance_opw00018(self, trcode, recordName):
        ...
        - 설정에 따라 보유종목을 trade_map + UI와 동기화
        - holdings-only 모드면 비보유(0수량) 종목은 자동 제거
        """
        try:
            ...
            sync_all_holdings = bool(getattr(self, "sync_balance_holdings_to_trade_list", False))
            for i in range(data_cnt):
                ...
                if code:
                    if sync_all_holdings or (code in self.trade_map):
            ...
            # 잔고 동기화 출처 코드 중 0수량/미등재 종목 자동 정리
            stale_codes = []
            ...
            # 체결(gubun=0)에서 임시로 생성된 0수량 유령 종목 정리
            ghost_codes = []
            for c, t in list(self.trade_map.items()):
                if c in seen_holding_codes:
                    continue
                if not bool(getattr(t, "auto_created_by_chejan", False)):
                    continue
                if int(getattr(t, "pos_qty", 0) or 0) > 0:
                    continue
                ghost_codes.append(c)
            if ghost_codes:
                removed_ghost, _ = self._remove_trade_codes(
                    ghost_codes,
                    reason="balance-chejan-ghost-auto-exclude",
                    block_holding=True
                )
                if removed_ghost > 0:
                    self.log(f"[BAL] chejan ghost auto-excluded={removed_ghost}")

            if self._is_holdings_only_mode():
                self._prune_non_holding_trade_items(reason="balance-holdings-only-prune")

            # 합계 필드가 비어 있으면 보유종목 집계값으로 보완
            if total_buy <= 0:
                total_buy = hold_buy
            if total_eval <= 0:
            ...
            if skipped_auto_sync > 0 and not sync_all_holdings:
                self.log(
                    f"[BAL] auto-sync OFF -> skipped Trade List add for holdings={skipped_auto_sync}"
                )
        except Exception as e:
    ...
    def _on_receive_tr_data(self, screenNo, rqname, trcode, recordName, prevNext, dataLen, errCode, msg1, msg2):
        ...
        if str(trcode).strip() == "opt10001":
            try:
                ...
                if code_real in self.trade_map:
                    self.trade_map[code_real].last = last
                    self.trade_map[code_real].last_price = last
        ...
        if not ctx:
            ...
            self.log(f"[TR IN] ctx-miss rq={rqname} tr={tr_name}")  # 컨텍스트 누락 TR 로그
            return
    ...
    def _parse_tr_3m_bootstrap(self, trcode: str, recordName: str, code: str, cnt: int):
        ...
        try:
            ...
            # 3분봉 파싱 예외 대응
            # =========================
            if not hasattr(t, "vol_hist_3m") or t.vol_hist_3m is None:
                t.vol_hist_3m = []
            if len(t.vol_hist_3m) != len(t.closes_3m):
                t.vol_hist_3m = [0] * len(t.closes_3m)
    ...
    def _set_realreg_for_trade_map(self):
        ...
        - 이미 등록되어 있어도 SetRealReg를 다시 호출할 수 있으므로 screen 번호 관리에 주의
        """
    ...
    def _on_receive_real_data(self, code, realType, realData):
        ...
        # 3분봉 버킷 업데이트
        finalized = self._update_3m_from_ticks(code, ts, last, vol)
    ...
    def _update_3m_from_ticks(self, code: str, epoch_sec: int, price: int, tick_vol: int = 0) -> bool:
        """
        실시간 틱으로 3분봉을 이어붙임.
        return:
            True  -> 직전 3분봉이 방금 확정됨
        ...
        if t.cur_3m_bucket == 0:
            ...
            t.cur_3m_vol = max(0, int(tick_vol))
            return False

        # 같은 3분봉 구간
        if bucket == t.cur_3m_bucket:
            t.cur_3m_close = float(price)
            t.cur_3m_vol = int(getattr(t, "cur_3m_vol", 0) or 0) + max(0, int(tick_vol))
            return False
        ...
        if not hasattr(t, "vol_hist_3m") or t.vol_hist_3m is None:
            t.vol_hist_3m = []
        t.vol_hist_3m.append(max(0, int(getattr(t, "cur_3m_vol", 0) or 0)))
        # 계산용 시리즈에도 동일 반영
        ...
        if len(t.closes_3m) > self.max_bars_3m:
            ...
            t.vol_hist_3m = t.vol_hist_3m[cut:] if len(t.vol_hist_3m) >= cut else []
    ...
    def _on_receive_chejan_data(self, gubun, itemCnt, fidList):
        ...
        # 체결 코드 자동 등록(조건 강화)
        if code not in self.trade_map:
            # gubun=0(주문/체결) 단독 이벤트는 과거 체결 재통보로 0수량 유령종목을 만들 수 있으므로
            # 신규 자동생성은 gubun=1(잔고)에서만 허용한다.
            if gubun != "1":
                self.log(f"[CHEJAN] skip auto-create code={code} gubun={gubun}")
                return

            pos_qty_probe = to_int(self._get_chejan(CHEJAN_FID["pos_qty"]))
            avg_probe = abs(to_float(self._get_chejan(CHEJAN_FID["avg_price"])))
            if not (pos_qty_probe > 0 and avg_probe >= 0):
                self.log(f"[CHEJAN] skip auto-create code={code} gubun={gubun} qty={pos_qty_probe}")
                return
        ...
        if gubun == "0":
            ...
            unfilled_qty = to_int(self._get_chejan(CHEJAN_FID["unfilled_qty"]))
            ...
            t.last_unfilled_qty = unfilled_qty

            self.log(f"[CHEJAN-0] {code} price={filled_price} qty={filled_qty} unfilled={unfilled_qty}")
            return
        ...
        # =====================================================
            ...
            # -------------------------
                ...
                t.pending_sell_qty = max(0, int(getattr(t, "pending_sell_qty", 0) or 0) - sold_qty)
                if int(getattr(t, "last_unfilled_qty", 0) or 0) == 0 and new_qty > 0:
                    # 주문 잔량이 없으면 부분청산 이후에도 pending 해제 가능
                    t.pending_sell_qty = 0
                ...
                if new_qty == 0:
                    ...
                    # 부분체결 중에는 pending 유지, 목표수량 소진 + 락 만료 시에만 해제
                    if t.pending_sell_qty <= 0 and time.time() >= float(getattr(t, "exit_lock_until", 0.0) or 0.0):
                        t.pending_sell = False
    ...
    def _check_entry_condition_3m(self, code: str, t: TradeItem):
        ...
        # 진입 조건 계산 보완
        # =========================
        market_tot = int(getattr(self, "market_up", 0)) + int(getattr(self, "market_down", 0))
        regime_ratio = float(getattr(self, "market_up_ratio", 0.5))
        if market_tot > 0 and regime_ratio < float(getattr(self, "market_regime_threshold", 0.52)):
            return
        if hasattr(t, "last_sell_time"):
            if (
                time.time() - float(getattr(t, "last_sell_time", 0.0) or 0.0) < 300
                and str(getattr(t, "last_exit_reason", "")) in {"STRUCTURE_BREAK", "EMA60_BREAK", "MAX_LOSS", "TRAILING_STOP", "EMA_DEAD_CROSS"}
            ):
                return
        ...
        if getattr(t, "last_entry_bucket", -1) == last_closed_bucket:
            return
        block_until = int(getattr(t, "reentry_block_until_bucket", -1) or -1)
        if block_until >= 0 and last_closed_bucket <= block_until:
            return
        ...
        if len(vols) >= 5:
            ...
            if avg_vol > 0 and vols[-1] < avg_vol * float(getattr(self, "volume_ratio_min", 1.30)):
                return
        ...
        if not (close_now > ema20_last > ema60_last):
            return
        chase_pct = (close_now - ema20_last) / ema20_last if ema20_last else 0.0
        if chase_pct > float(getattr(self, "max_chase_pct", 0.02)):
            return
    ...
    def _check_eod_profit_exit_or_hold(self, code: str, t: TradeItem):
        # ARM OFF(모의) 상태에서는 EOD 강제청산으로 로컬 포지션을 훼손하지 않는다.
        if not bool(getattr(self, "arm_live", False)):
            return
    ...
    def _sell_market(self, code: str, qty: int, reason: str = ""):
        ...
        if not t or qty <= 0:
            return
        qty = min(int(qty), int(getattr(t, "pos_qty", 0)))
        if qty <= 0:
            return

        now_ts = time.time()
        lock_until = float(getattr(t, "exit_lock_until", 0.0) or 0.0)
        if now_ts < lock_until:
            self.log(f"[SELL-SKIP] {code} exit-lock active ({lock_until - now_ts:.1f}s) reason={reason}")
            return
        ...
        if t.pending_sell:
            ...
            return
        if reason and getattr(t, "last_exit_reason", "") == reason and (now_ts - float(getattr(t, "last_exit_ts", 0.0) or 0.0)) < float(getattr(self, "exit_lock_sec", 3.0)):
            self.log(f"[SELL-SKIP] {code} same-reason throttle reason={reason}")
            return
        ...
        # 매도 주문 전 pending_sell 설정
        t.pending_sell = True
        t.pending_sell_qty = qty
        t.exit_lock_until = now_ts + float(getattr(self, "exit_lock_sec", 3.0))
        t.last_exit_reason = reason
        t.last_exit_ts = now_ts
        if reason in {"STRUCTURE_BREAK", "EMA60_BREAK", "MAX_LOSS", "TRAILING_STOP", "EMA_DEAD_CROSS"}:
            cur_bucket = int(getattr(t, "cur_3m_bucket", 0) or 0)
            if cur_bucket > 0:
                cd_bars = int(getattr(self, "reentry_cooldown_bars", 8) or 0)
                t.reentry_block_until_bucket = cur_bucket + (cd_bars * 180)
                t.last_exit_bucket = cur_bucket
        ...
        if not self.arm_live:
            try:
                if self.SIM_FILL_WHEN_ARM_OFF:
                    # 잔고 동기화로 유입된 실제 보유는 ARM OFF에서 모의 매도로 수량을 바꾸지 않는다.
                    if bool(getattr(t, "synced_from_balance", False)) and not bool(getattr(t, "manual_picked", False)):
                        self.log(f"  -> ARM LIVE OFF: skip SIM SELL for balance-synced holding {code}")
                        return
